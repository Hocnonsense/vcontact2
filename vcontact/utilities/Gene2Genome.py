...
options.add_argument('-s', '--source-type', dest='source_type', choices=[
    'VIRSorter', 'Prodigal-coords', 'Prodigal-FAA', 'MetaGeneMark', 'NCBICodingSequence', 'NCBIFasta'],
                     required=True, help='Select one of the options as an input source. MetaGeneMark can be either the '
...
if __name__ == "__main__":
    ...
    with open(results.output, 'w') as proteins_fh:
        ...
        if any(source in results.source_type for source in sources):
            ...
            for contig, genes in gene2contig.items():
                for gene in genes:
                    csvWriter.writerow([gene, contig, 'None'])