...
options.add_argument(
    ...
    dest="proteins",
    metavar="FILENAME",
    default="VIRSorter_prots.fasta",
)
options.add_argument(
    "-o", "--output", dest="output", metavar="FILENAME", default="proteins.csv"
)
options.add_argument(
    "-s",
    "--source-type",
...
def parseProdigalCoords(proteins_fh):
    ...
    for tokens, start_pos, end_pos in text.scanString(content):  # Tokens[0] is start
        headerDict = dict(
            ele.split("=") for ele in tokens[1].split("\n")[0].strip().split(";")
        )

        contig = str(headerDict["seqhdr"].strip('"'))

        relDict.setdefault(contig, [])

        data = tokens[1].strip().split("\n")[1:]
        cdsData = [
            ele.strip().split('/note="')[-1] for ele in data[2:] if "CDS" not in ele
        ]
...
def parse_NCBI_coding(proteins_fh):
    """"""
    relDict: dict[str, list[tuple[str, str]]] = {}
    ...
    for record in SeqIO.parse(proteins_fh, "fasta"):
        ...
        relDict.setdefault(contig, []).append((proteinID, proteinInfo))
