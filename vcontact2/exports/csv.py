...
def summary(folder, gc=None, pcm=None, category="RefSeq-85"):
    ...
    if gc is not None:
        ...
        output.append(
            ("{tot} Contigs: {nb_cat} in {cat} and {nb_ncat} not in {cat}" "").format(
                nb_ncat=len(pos_tara),
                nb_cat=len(pos_refseq),
                tot=len(pos_tara) + len(pos_refseq),
                cat=category,
            )
        )

        clusters_refseq = frozenset(
            contigs.loc[pos_refseq, "pos_cluster"].drop_duplicates()
        )
        clusters_tara = frozenset(
            contigs.loc[pos_tara, "pos_cluster"].drop_duplicates()
        )
        print(
            (
                "{tot} Clusters: {nb_cat} with a {cat} sequence "
    ...
    if pcm is not None:
        ...
        refseq_pcs = (
            np.squeeze(np.asarray(pcm.matrix[pos_refseq, :].tocsc().sum(0))) > 0
        ).sum()
        tara_pcs = (
            np.squeeze(np.asarray(pcm.matrix[pos_tara, :].tocsc().sum(0))) > 0
        ).sum()
        commons_pcs = int(
            np.dot(
                np.asarray(
                    (np.asarray(pcm.matrix[pos_refseq, :].tocsc().sum(0)) > 0),
                    dtype=int,
                ),
                np.asarray(
                    np.transpose(
                        (np.asarray(pcm.matrix[pos_tara, :].tocsc().sum(0)) > 0)
                    ),
                    dtype=int,
                ),
            )
        ...
        print(
            (
                "{tot} shared PCs (+ {sing}, for a total of {allpcs}): "
    ...
    fp = os.path.join(
        folder, "sig{}_mcl{}{}".format(gc.thres, gc.inflation, permissive)
    )
    with open(fp, "w") as f:
        f.write("\n".join(output))
